...
func NewBGPController(cfg Config) (*controller, error) {
	asn := cfg.ASN; if asn == 0 { asn = 65001 }
	mgr := frr.NewVTYSH(frr.Config{ASN: asn, EnsureStatic: cfg.EnsureStatic, VTYSHPath: cfg.VTYSHPath})
	c := &controller{ cfg: cfg, frr: mgr, desired: map[string]bool{}, announced: map[string]bool{} }
	cfg.ServiceInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(interface{}, interface{}){ go c.ReconcileOnce(context.Background()) },
...
func (c *controller) Run(ctx context.Context) {
	t := time.NewTicker(c.cfg.LoopInterval); defer t.Stop()
	for {
		select { case <-ctx.Done(): return; case <-t.C: c.ReconcileOnce(ctx) }
	}
}

func (c *controller) WithdrawAll(ctx context.Context) {
	c.mu.Lock()
	keys := make([]string, 0, len(c.announced)); for k := range c.announced { keys = append(keys, k) }
	c.mu.Unlock()
	for _, k := range keys {
		ip, pfx := parseCIDRKey(k)
		if err := c.frr.WithdrawVIP(ip, pfx); err != nil { log.Printf("withdraw %s error: %v", k, err) } else { metrics.VIPWithdrawn.WithLabelValues("*","*", ipFamily(ip), c.cfg.NodeName).Inc() }
	}
}

func (c *controller) ReconcileOnce(ctx context.Context) {
	defer func(){ recover() }()
	c.computeDesired(ctx); c.apply(ctx)
}
...
func (c *controller) computeDesired(ctx context.Context) {
	...
	for _, cond := range node.Status.Conditions {
		if cond.Type == corev1.NodeNetworkUnavailable && cond.Status == corev1.ConditionTrue { nodeDraining = true }
	}
	for _, svc := range svcs {
		var annGate *bool
		if v, ok := svc.Annotations["cosmolet.platformbuilds.io/announce"]; ok { b := strings.ToLower(v)=="true"; annGate=&b }
		...
		for _, es := range eps { if es.Labels[discoveryv1.LabelServiceName] == svc.Name { esForSvc = append(esForSvc, es) } }
		policy := PolicyAuto
		...
		} else { metrics.EndpointsReady.WithLabelValues(svc.Name, svc.Namespace, c.cfg.NodeName).Set(0) }
	}
	c.mu.Lock(); c.desired = desired; c.mu.Unlock()
}

func ipFamily(ip net.IP) string { if ip.To4()!=nil { return "ipv4" }; return "ipv6" }

func parseCIDRKey(k string) (net.IP, int) {
	parts := strings.Split(k, "/"); pfx := 32
	if strings.Contains(parts[0], ":") { pfx = 128 }
	if len(parts)==2 { if n, err := strconv.Atoi(parts[1]); err==nil { pfx = n } }
	return net.ParseIP(parts[0]), pfx
}

func (c *controller) apply(ctx context.Context) {
	c.mu.Lock()
	desired := map[string]bool{}; for k,v := range c.desired { desired[k]=v }
	announced := map[string]bool{}; for k,v := range c.announced { announced[k]=v }
	c.mu.Unlock()

	for k := range desired {
		if !announced[k] {
			ip, pfx := parseCIDRKey(k)
			if err := c.frr.AnnounceVIP(ip, pfx); err != nil { metrics.ReconcileErrors.Inc(); log.Printf("announce %s failed: %v", k, err); continue }
	...
	for k := range announced {
		if !desired[k] {
			ip, pfx := parseCIDRKey(k)
			if err := c.frr.WithdrawVIP(ip, pfx); err != nil { metrics.ReconcileErrors.Inc(); log.Printf("withdraw %s failed: %v", k, err); continue }
			metrics.VIPWithdrawn.WithLabelValues("*","*", ipFamily(ip), c.cfg.NodeName).Inc()
			delete(announced, k)
		}
	}
	c.mu.Lock(); c.announced = announced; c.mu.Unlock()
}
